...
const addTodo = () => {
  ...
  router.post(route('userToDoList.store'), {
    title: newTodo.value,
...
const updateTodoStatus = (todo) => {
   router.put(route('userToDoList.update', todo.id), {
...
const submit = () => {
  editForm.put(route('userToDoList.update', editForm.id), {
    onSuccess: () => {
    editForm.reset();
...
const deleteToDoList = () => {
  deleteForm.delete(route('userToDoList.destroy', deleteForm.id), {
    onError: (errors) => {
